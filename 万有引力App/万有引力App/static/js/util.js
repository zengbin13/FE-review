...
const util = {
	...
	//获取image集合
	getImageList(imgStr, serviceImgUrl) {
		serviceImgUrl = serviceImgUrl || '';
		let array = imgStr ? imgStr.split('>>') : [];
		for (var i = 0; i < array.length; i++) {
			array[i] = serviceImgUrl + array[i]
		}
		return array;
	},
	//是否为空字符串
	...
	getUniPush() {
		...
		data.package = 'com.yunfanda.gravitation';
	...
	requestPayment(payCode, orderInfo) {
		// console.log('orderInfo.start', orderInfo);
		let provider = '';
		if (payCode == 0) {
			provider = 'alipay';
