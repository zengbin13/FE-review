			...
			<view class="circle_list" :class="{cur:discountChecked}">
				...
				<view v-if="discountChecked" style="margin: 34rpx 0;">
					<view class="free_theme_price cl">
						<view class="num" v-for="(item,index) in discountArray" :key="index" :class="{'cur':item==discountCur}" @tap="discountTap(item)">{{item}}折</view>
					</view>
				</view>
			</view>
			<view class="circle_btn" @tap="toPay">去创建</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async toPay() {
				...
				let param = this.circleParam;
				param.rules = unitsJson;
				// param.price = price;
		...
		onLoad() {
			this.circleParam = uni.getStorageSync('circle_add_param');
			uni.removeStorageSync("circle_add_param")
		}
	}
</script>
