<template>
	<view>
		...
		<view class="cirFree_itm">
			...
			<view class="cirFree_btn" @tap="complete" :class="{'cur':checkData}">完成创建</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			//初始化
			async init() {
				let res = await this.$service.circle.info({
					circleId: this.circleId
				})
				if (res.success) {
					// console.log(res);
					let circleInfo = res.data.data.circleInfo;
					this.circleBackgroundImg = circleInfo.circleBackgroundImg;
					this.circleIntro = circleInfo.circleIntro;
					this.circleTitle = circleInfo.circleTitle;
					this.circleLogo = circleInfo.circleLogo;
					this.isShow = !!this.circleBackgroundImg;
				}
			},
			//提交
			async complete() {
				...
				var res = await this.$service.circle.update(param);
