<template>
	<view class="lb-multi-selector lb-picker-item" :style="{ height: height }">
		<picker-view :value="pickerValue" :indicator-style="indicatorStyle" :style="{ height: height }" @change="handleChange">
			<picker-view-column v-for="(column, index) in pickerColumns" :key="index">
				...
				<view v-for="(item, i) in column || []" :class="[
				 ...
				 :key="i" :data-item="pressEnable ? JSON.stringify(item) : ''" @touchstart="touchstart" @touchmove="touchmove"
				 @touchend="touchend">
					<!-- #endif -->
					<!-- #ifndef H5 -->
					<view v-for="(item, i) in column || []" :class="[
            ...
            item[props.value] === selectValue[index]
              ...
              : ''
					 ...
					 :key="i" :data-item="item" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend">
						<!-- #endif -->
						<!-- #ifdef APP-PLUS || H5 -->
						<text :class="[
