		...
		<view class="content">
			...
			<view class="other_login cuIcon">
				<view class="login_icon">
					<view class="cuIcon-weixin wechat_color" @tap="loginType(0)"></view>
				</view>
				<!-- <view class="login_icon">
			...
			<!-- 底部信息 -->
			<view class="footer">
				<view v-if="loginMode === 0" @tap="toggleLoginMode(1)">密码登录</view>
				<view v-if="loginMode === 1" @tap="toggleLoginMode(0)">账号注册</view>
				<!-- <navigator url="register" open-type="navigate"></navigator> -->
				<text>|</text>
	...
	export default {
		...
		methods: {
				...
				try {
					...
					if (token) {
						...
						if(userInfo) {
							...
							this.$storage.removeSync('token')
							// 提示过期
							this.$utils.showToast('登录已过期')
							return
						}
			...
			async codeLogin() {
				...
				let res = await this.$service.login.login(params)
				//登录
				if(res.data.code === 0) {
					// 跳转到index
			...
			async passLogin() {
				...
				let res = await this.$service.login.pass_login(params)
				if(res.data.code === 0) {
					this.logined(res.data.data)
				}
			},
			...
			uploadInfo(data) {
				...
				let mobile = data.mobile || this.phoneData
				// 跳转到完善资料页
				uni.redirectTo({
					...
					success: (res) => {
						if(type === 0 && ~res.provider.indexOf('weixin')) {
							...
							uni.login({
								provider: 'weixin',
								success: (loginRes) => {
									let key = loginRes.authResult.openid
									this.thirdPartylogin(key, 1)
								},
								fail: () => {
									this.$utils.showToast("获取微信用户数据失败")
								}
							})
						}
					}
			...
			async thirdPartylogin(key, type) {
				let params = {
					third_key: key,
					third_type: type
				}
				let res = await this.$service.login.third_party_login(params)
				if(res.data.code === 4) {
					//存在账号 但首次微信登录
					let aid = res.data.data.aid
					uni.navigateTo({
