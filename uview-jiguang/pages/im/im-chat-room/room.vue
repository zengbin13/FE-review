	...
	<view>
		...
		<view class="material-apply" v-show="showMenu">
			<view class="apply-list">
				...
				<view class="apply-item" @click="showDialog3">设置用户禁言</view>
			</view>
		</view>
		...
		<view class="cu-modal" :class="show3?'show':''">
			<view class="cu-dialog">
				...
				<view class="padding-xl">
					<view class="cu-form-group margin-top text-left">
						<input v-model="silenceUsername" placeholder="输入禁言用户名" />
					</view>
				</view>
				<view class="cu-bar bg-white justify-end">
					<view class="action">
...
<script>
	...
	export default {
		...
		computed: {
			...mapState(['hasLogin', 'loginProvider', 'nickname', 'avatar', 'receiveChatRoomMessage', 'sendMessageResponse']),
			emojiList: {
				get: function() {
		...
		onLoad(option) {
			...
			this.jpushIM.ChatRoom.enterChatRoom({"roomId":this.roomId}, (callback) => {
				if(callback.errorCode == 0){
					console.log("JMessagePlugin enterChatRoom",callback.data);
					uni.showModal({
						title: '进入聊天室',
						content: JSON.stringify(callback.data),
						showCancel: false,
						cancelText: '',
						confirmText: '关闭'
					});
			...
			// let param = {"roomIds":roomId};
			// this.jpushIM.ChatRoom.getChatRoomInfoListById(param, (callback) => {
			// 	if(callback.errorCode == 0){
			// 		console.log("JMessagePlugin getChatRoomInfoListById",callback.data);
			// 		uni.showModal({
			// 			title: '聊天室信息',
			// 			content: JSON.stringify(callback.data),
			// 			showCancel: false,
		...
		onUnload() {
			...
			this.jpushIM.ChatRoom.exitChatRoom(param, (callback) => {
				if(callback.errorCode == 0){
					console.log("JMessagePlugin exitChatRoom",callback);
					uni.showModal({
						title: '离开聊天室',
		...
		methods: {
			...
			sendMsg(content, type) {
				...
				switch (type) {
					case "text":
						params = {
							"msgType": type,
							"type":"chatRoom",	// 消息类型，'single' / 'group' / 'chatRoom'
			...
			confirm3:function(){
				...
				uni.showModal({
					title: '提示',
					content: '是否确认禁言用户：' + this.silenceUsername,
					success: (res) => {
						if (res.confirm) {
							...
							let params = {
								"groupId": this.groupId,
								"isSilence": true,
								"username": this.silenceUsername,
							}
							this.jpushIM.setGroupMemberSilence(params, (callback) => {
								console.log(callback);
								uni.showModal({
									title: callback.errorCode == 0?'成功':'错误',
									content: callback.errorMsg,
		...
		watch: {
			receiveChatRoomMessage(res) {
				// console.log("监听聊天室消息事件");
				uni.showModal({
					title: '聊天室新消息 自行扩展',
