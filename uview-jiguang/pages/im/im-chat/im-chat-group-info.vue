<template>
	<view>
		...
		<view class="bg-white">
			...
			<view class="cu-form-group margin-top">
				<view class="title" @click="groupSilenceMembers">获取禁言用户列表</view>
			</view>
		...
		methods: {
			init:function(){
				...
				this.jpushIM.getGroupInfo({"id":this.groupId}, (callback) => {
					...
					this.avatar = groupInfo.avatar;	// 群头像
				})
			...
			chooseImage: function() {
				uni.chooseImage({
					...
					success: (res) => {
						...
						// 将本地URL路径转换成平台绝对路径（这一步不能省）
						imageSrc = plus.io.convertLocalFileSystemURL(imageSrc);
						...
						this.jpushIM.updateGroupAvatar(params, (callback) => {
							uni.hideLoading();
			...
			groupSilenceMembers:function(){
				// #ifdef APP-PLUS
				this.jpushIM.groupSilenceMembers({
						groupId: this.groupId,
					},
					(callback) => {
						uni.showModal({
