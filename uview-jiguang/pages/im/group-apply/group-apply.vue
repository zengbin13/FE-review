	...
	<view>
		...
		<block v-if="hasLogin === true">
			...
			<view v-for="(item, index) in listData" :key="index">
				...
				<view class="friend_item cu-list menu-avatar" v-for="(user,index2) in item.joinGroupUsers" :key="index2">
					
					<view class="cu-item">
						<view class="cu-avatar round lg" :style="'background-image:url('+ user.avatar +');'">
							<!-- <view class="cu-tag badge">{{user.gender}}</view> -->
						</view>
						<view class="content">
							<view class="text-grey">
								<view class="text-cut">{{user.nickname}}(帐号：{{user.username}}）)</view>
								<view class="cu-tag round bg-orange sm">性别：{{user.gender}}</view>
							</view>
					...
					<view class="padding bg-white">
						...
						<block v-else>
							<!-- 待处理 -->
							<view class="flex">
								<view class="flex-sub bg-green padding-sm margin-xs radius" @click="agree(index)">同意</view>
								<view class="flex-sub bg-red padding-sm margin-xs radius" @click="refuse(index)">拒绝</view>
							</view>
