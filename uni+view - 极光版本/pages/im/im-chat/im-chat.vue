			...
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop"
			 :scroll-into-view="scrollToView" upper-threshold="50" :style="[{top:CustomBar + 'px'}]">
				...
				<view class="row" v-for="(row,index) in msgList" :key="index" :id="row.msgId">
					...
					<view class="system" v-if="row.messageType == 'prompt'">
						<!-- 消息撤回 -->
						<view class="text">{{row.promptText}}</view>
					</view>
		...
		<view class="popup-layer" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<view class="more-layer" :class="{hidden:hideMore}">
				<view class="list">
					...
					<view class="box" @tap="chooseFiles">
		...
		<view class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<view class="textbox">
				...
				<view class="text-mode" :class="isVoice?'hidden':''">
					<view class="box">
						<!-- 感谢 @呵呵哒 提供适配方案 -->
						<textarea auto-height="true" cursor-spacing="10" v-model="textMsg" @focus="textareaFocus" />
						</view>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState(['hasLogin', 'loginProvider', 'nickname', 'avatar', 'receiveMessage', 'sendMessageResponse','retractMessage']),
			emojiList: {
				get: function() {
		...
		methods: {
			...
			setMessageHaveRead(id){
				...
				// 	complete: () => {}
				// });
				// console.log("JMessagePlugin setMessageHaveRead",params);
				this.jpushIM.setMessageHaveRead(params, (res) => {
			...
			screenMsg(msg) {
				...
				switch (msg.messageType) {
					...
					case 'voice':
						// 语音消息
						// console.log("JMessagePlugin push一条语音消息 " + JSON.stringify(msg))
			...
			setSingItem: function(item) {
				...
				if (item.messageType == "text") {
					...
					// 文件消息，原本应该是绝对路径转平台url
					// 由于5+限制，无法读取，抓网络地址拼接，图片加载会有些慢
					// console.log("读取路径：JMessagePlugin img string： " + plus.io.convertAbsoluteFileSystem(item.thumbPath));
					// console.log("JMessagePlugin 文件消息setSingItem",item);
					content = {
						url: item.messageString,
						fileName:item.fileName,	//文件名
					}
					// console.log("JMessagePlugin 文件消息",item);
					// console.log("JMessagePlugin示例下载文件");
					// this.jpushIM.downloadFile({
					// 	"type":"single",
					// 	"username":this.chatUsername,
					// 	"messageId":item.messageId
					// }, (res) => {
					// 	console.log("JMessagePlugin 示例下载文件",res);
					// });
					// console.log("JMessagePlugin 文件消息",content);
					
				} else if (item.messageType == "prompt") {
					// 消息撤回
				...
				item.originCreateTime = item.createTime; //v2.4 将原始创建时间保存下来
						...
						for (let i = 0; i < res.tempFilePaths.length; i++) {
							uni.getImageInfo({
								...
								success: (image) => {
									...
									let msg = {
										url: res.tempFilePaths[i],
										fileName: res.tempFiles[i].name, //文件名
									};
			...
			sendMsg(content, type) {
				...
				switch (type) {
					...
					case "image":
						...
						// console.log("JMessagePlugin 图片消息传参：" + JSON.stringify(params));
						break;
					case "voice":
						// 语音消息
						params = {
						...
						// console.log("JMessagePlugin 发送语音消息传参：" + JSON.stringify(params));
						break;
					case "file":
						// 文件消息
						params = {
						...
						console.log("JMessagePlugin 发送文件消息传参：" + JSON.stringify(params));
						break;
					case "location":
						params = {
							"msgType": type,
							"type": "single", // 消息类型，'single' / 'group' / 'chatRoom'
							"username": this.chatUsername, // 接收人
							"latitude": content.latitude,	// 注意是Number
							"longitude":content.longitude,	// 注意是Number
							"scale":content.scale,	// 注意是Number
							"address":content.address,	// 注意是String
							"extras":content.extras	// 注意是object
						};
						console.log("JMessagePlugin 发送地理位置消息传参：" + JSON.stringify(params));
						break;
					default:
						break;
				}
				...
				this.$store.dispatch("submitChatMsg", params).then(response => {
					...
					// 发送消息
					// console.log("JMessagePlugin sendMsg发送消息回调",msg);
					this.screenMsg(msg);
				}, response => {
					// 失败
					uni.showModal({
			...
			textMessageTouch(index){
				...
				if (!msg.createTime) {
					...
					console.log("JMessagePlugin isThreeMin",isThreeMin);
					if (isThreeMin) {
						this.$set(msg,"showRetractBtn",true);//显示撤回按钮
					}else{
						uni.showModal({
							title: '警告',
							content: '不在三分钟内的消息，无法操作',
							showCancel: false,
							cancelText: '',
							confirmText: '关闭',
							success: res => {},
				...
				this.$set(msg,"forwardMessageBtn",true);//显示转发按钮
				
				this.$set(msg,"forwardMessageToGroupBtn",true);//显示转发到群按钮
				
				// console.log(msg.content);
				// console.log(msg.showRetractBtn);
				// console.log(index);
			},
			// 撤回消息
			retract(index){
				// 注意事项，超过2分钟的不可撤回，自行加一个判断，后续优化再补
				const msg = this.msgList[index];
				
				let param = {
					'type': 'single',       // 'single' / 'group' / 'chatRoom'
					'username': this.chatUsername,
					'messageId': msg.messageId
				}
				// console.log("JMessagePlugin retract传参",param);
				this.jpushIM.retractMessage(param, (res) => {
					// 撤回成功
					if(res.errorCode == 0){
						uni.showToast({
							title: '撤回成功',
			...
			forward(index){
				...
				let param = {
					...
					'username': this.chatUsername,	
					'messageId': msg.messageId,
					'toUsername':'Yoyo01',// 测试转发消息，默认为Yoyo01，需自行添加页面指定人或群
				}
				// console.log("JMessagePlugin forward传参",param);
				this.jpushIM.forwardMessage(param, (res) => {
					// 没有转发失败的回调，只有成功
					uni.showModal({
						title: '提示',
						content: "转发成功",
			...
			forwardToGroup(index){
				...
				let param = {
					...
					'username': this.chatUsername,	
					'messageId': msg.messageId,
					'toGroupId':'42956423',// 测试转发到群示例，测试时，群号请自行更改
				}
				// console.log("JMessagePlugin forward传参",param);
				this.jpushIM.forwardMessage(param, (res) => {
					// 没有转发失败的回调，只有成功
		...
		watch: {
			receiveMessage(res) {
				...
				if (res.fromUser.username == this.chatUsername) {
					...
					this.screenMsg(msg);
					this.setMessageHaveRead(msg.messageId); //设置消息已读，同时如果发送消息设置needReadReceipt，则可以获取已读回执
				}
			},
			...
			retractMessage(retractRes) {
				// 监听消息撤回事件
				// console.log("JMessagePlugin retractMessage",retractRes);
				let res = retractRes.retractedMessage;
				const msg = this.setSingItem(res);
				var list = this.msgList;
				for (let i = 0; i < list.length; i++) {
					if (list[i].messageId == res.messageId) {
						console.log("JMessagePlugin retractMessage msg 信息变更",msg);
						this.$set(list,i,msg);//信息变更
					}
				}
			}
		},
