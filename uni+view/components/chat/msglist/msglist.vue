<template>
	<view scroll-y="true" :class="view + ' wrap ' + (isIPX ? 'scroll_view_X' : '')" @tap="onTap" upper-threshold="-50" :scroll-into-view="toView">
		<view class="message" v-for="item in chatMsg" :key="item.mid" :id="item.mid">
			...
			<view class="main" :class="item.style">
				...
				<image class="avatar" :src="avatarUrl(item)" />
				<view class="msg">
					<image class="err" :class="item.style == 'self' && item.isFail ? 'show' : 'hide'" src="/static/images/msgerr.png" />

					<image v-if="item.style == 'self'" src="/static/images/poprightarrow2x.png" class="msg_poprightarrow" />
					...
					<view v-else-if="item.msg.type == 'txt' || item.msg.type == 'emoji'">
						<view class="template" v-for="(d_item, d_index) in item.msg.data" :key="d_index">
							<text :data-msg="item" @tap="clickMsg" v-if="d_item.type == 'txt'" class="msg-text" style="float:left;">{{ d_item.data }}</text>
...
export default {
	...
	methods: {
		avatarUrl(item) {
			if(item.style === 'self') {
				return userInfo.avatar || '/static/images/theme2x.png'
			} else {
				return item.msg.ext.headImageUrl || '/static/images/theme2x.png'
			}
		},
		normalScroll() {
			this.setData({
		...
		clickMsg(event) {
			...
			if (typeof event.target.dataset.msg == 'object' && event.target.dataset.msg.msg.ext && event.target.dataset.msg.msg.ext.msg_extension) {
